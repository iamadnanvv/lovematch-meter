    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md bg-gradient-to-br from-background via-background to-rose-light border-primary/20 max-h-[85vh] overflow-y-auto">
        ...
        <div className="py-4 space-y-4">
          ...
          <ShareButton
            getText={() => {
              const allPredictions = Object.entries(PREDICTIONS_BY_YEAR)
                .map(([year, _]) => `${YEAR_EMOJIS[year]} ${year}: ${getPrediction(year)}`)
                .join('\n');
              return `⏰ Our Future Together (${player1Name} & ${player2Name}):\n\n${allPredictions}\n\n💕 Predicted by Love Triangle!`;
            }}
          />
